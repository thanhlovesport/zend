<?php
	...
	foreach ($menu as $key => $value){
		if($value['parents'] == 0){ // giá trị value phần từ parent
			$value['level'] = 1; 
			$newArray[] 	= $value; // mảng mới bằng nhữn giá trị mới có phần tử level = 1
			unset($menu[$key]);
	...
	foreach($newArray as $key => $value){
		if($value['level']==1){
			...
			$padding = 'padding-left: '.$padding.'px;'; // cách ô bên trong
